...
class Regression(nn.Module):
    ...
    def __init__(self,dict_params):
        ...
        for layer_no in range(conv_num):
            ...
            layers.append(nn.Conv1d(last_out, conv1d_out_channels, conv1d_kernel_size, 1))
            ...
            if pool_type == "avg":
                layers.append(nn.AvgPool1d(conv_pool))
            elif pool_type == "max":
                layers.append(nn.MaxPool1d(conv_pool))
            else:
                conv_pool = 0
            ...
            if conv_pool is not 0:
                last_input = int((last_input-conv1d_kernel_size+1)/conv_pool)
            else:
                last_input = (last_input - conv1d_kernel_size + 1)
...
class PolicyGradientNetwork(nn.Module):
    ...
    def __init__(self,state,architecture_map=None,hidden_size=64,max_layer=6):
        super().__init__()
        ...
        self.conv_layer =  nn.Linear(state_size, 2)
        self.linear_layer =  nn.Linear(state_size, 2)
        self.lstm1 = nn.GRUCell(
    ...
    def forward(self, state,conv_layer=2,linear_layer=2):
        ...
        # h_t, c_t = self.lstm1(state, (h_t, c_t))
        h_t = self.lstm1(state, h_t)
        hid = h_t.squeeze()
        result = self.conv_layer(hid)
        conv_num_softmax = F.softmax(result, dim=-1)
        action_prob_map["conv_num"] = conv_num_softmax#torch.FloatTensor([0.333,0.444,0.222])

        # h_t, c_t = self.lstm1(state, (h_t, c_t))
        # hid = h_t.squeeze()
        result = self.linear_layer(state)
        linear_num_softmax = F.softmax(result, dim=-1)
        action_prob_map["linear_num"] = linear_num_softmax# torch.FloatTensor([0.222,0.222,0.122,0.122,0.222,0.122])
...
class PolicyGradientAgent():
    ...
    def __init__(self):
        ...
        self.conv_num = 2
        self.linear_num = 2
        self.activation_functions_list = ['Sigmoid','Tanh','ReLU','LeakyReLU','None']
        self.pool_type_list = ['avg','max','none']
        self.pool_kernel_list = [2,3]
        self.conv1d_out_channels_list = [i for i in range(100) if i>9 and i%5==0] #[80,90,100,110]
        self.conv1d_kernel_size_list = [1,3,5,7]  #[7,5,1]
        self.drop_out_list = [0,0.1,0.2,0.3]
        self.conv_num_list = [1,2,3]
        self.linear_num_list = [1,2,3,4,5,6]
        ...
        self.network = PolicyGradientNetwork(tp_size,self.architecture_map,max_layer=6).to(device)
...
def reinforcementlearning_main():
    ...
    NUM_BATCH = 7600  # 總共更新 7600 次

    hidden_size = 64
    state = torch.rand(1, 64,dtype=torch.float, device=device)#torch.zeros(1, hidden_size, dtype=torch.float, device=device)
    ...
    for batch in range(NUM_BATCH):
        ...
        actionparam,log_prob,state = agent.sample(state)
        ...
        mean_loss = 566#(action_loss+evaluate_loss)/2
...
if __name__ == '__main__':
    ...
    network = PolicyGradientNetwork(64, {}, max_layer=6).to(device)

    optimizer = optim.SGD(network.parameters(), lr=0.001)
    state = torch.rand(1, 64, dtype=torch.float,
                       device=device)  # torch.zeros(1, hidden_size, dtype=torch.float, device=device)
    network.train()
    for i in range(5000):
        action_prob_map, state = network(state)
        mean_loss = 566#(action_loss+evaluate_loss)/2
        reward = 480 - mean_loss
        # print("reward:",reward,"mean_loss:",mean_loss," action_loss",action_loss,"evaluate_loss",evaluate_loss)
        loss = -torch.mean(state) * reward
        print("reinfor loss ", loss)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()