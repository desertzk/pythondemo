...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("debugpadding7.log"),
...
class PolicyGradientAgent():
    ...
    def __init__(self):
        ...
        self.optimizer = optim.SGD(self.network.parameters(), lr=0.08)
    ...
    def learn(self, rewards,log_prob):
        # 损失函数要是一个式子
        x = -torch.mean(log_prob)*rewards/500
        loss = pow(0.5,x)
        logging.info(" x : "+str(x)+" reinfor loss "+str(loss))
...
def reinforcementlearning_main():
    ...
    for batch in range(NUM_BATCH):
        ...
        try:
            ...
            reward = 600 - mean_loss + spearman_reward
            global max_reward
            global max_spearman
            ...
            if reward < -1500 or math.isnan(reward):
